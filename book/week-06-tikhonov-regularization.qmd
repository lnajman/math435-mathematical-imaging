...
The three preceding ideas now meet:

| Ingredient | Source in the course | Role in Tikhonov |
|---|---|---|
| forward model | image formation and degradation | predict $Ax$ from $x$ |
| noise model | Chapter 4 | justify $\|Ax-y\|_2^2$ |
| ill-posedness | Chapter 5 | explain why data fit alone is unsafe |
| prior | this chapter | choose and stabilize a reconstruction |

This is why Tikhonov is more than a historical method. It is the simplest complete laboratory for the course identity:
...
The image model is intentionally simple here. Later chapters will replace it with edge-aware, sparse, multiscale, or learned models, but the logical structure remains the same.
...
In the notation of the course template,
...
The first term asks the reconstruction to explain the data under a Gaussian noise model. The second term expresses a prior preference for small-energy images. The parameter $\lambda$ decides how strongly that prior is allowed to influence the answer.
...
This limitation is pedagogically useful. Because the prior is so simple, we can see exactly what it buys and what it costs. It buys stability. It costs bias toward low-energy, often over-smoothed reconstructions. More advanced priors will try to buy stability while losing less important image structure.
...
This interpretation also prepares the neural-network part of the course. A learned prior can be very strong: it may know a great deal about typical images in a training set. But strength does not automatically mean truth. If the learned prior is trusted too much, the reconstruction may become visually convincing while moving away from the measurement. If the data term is trusted too much, the reconstruction may fit noise or unstable directions. The old Tikhonov question remains, even when the prior is learned: how much should the algorithm trust the data, and how much should it trust the image model?
