...
this chapter changes the meaning of $R$. The prior no longer says only "be small" or "be smooth." It says: be simple in a chosen representation. That representation might be DCT, wavelets, a learned dictionary, or later a neural feature space.

This is why sparse reconstruction belongs between TV and learned imaging. It teaches that the quality of a prior depends not only on the penalty, but also on the coordinates in which the penalty is applied.
...
## Sparsity Is Evidence Only With A Model

A sparse reconstruction is persuasive only when the sparse model is appropriate.

The statement "this image has a sparse representation" is incomplete. We must ask:

- sparse in which representation?
- under which measurement operator?
- at what noise level?
- with what regularization weight?
- for which image class?

If the representation matches the image and the measurements are informative enough, sparsity can strongly stabilize reconstruction. If the representation is mismatched, sparsity can select the wrong explanation with great confidence.

This is the same lesson as before, now in coefficient language. The data term checks the measurement. The sparse prior chooses among images that fit the measurement. The reconstruction should not be trusted unless both parts are reasonable.
...
This is the chapter where the prior moves from "the image should be smooth" to "the image should be simple in the right coordinates." That change matters. A neural network can later be understood as learning coordinates, filters, or features in which the reconstruction task becomes easier. Sparse reconstruction is the interpretable version of that idea.
...
A fourth mistake is to forget that sparsity is a claim about coefficients, not automatically a claim about truth in the scene.
...
7. Explain why sparse coefficients are useful evidence only after a representation and measurement model have been chosen.
...
- Sparsity can stabilize reconstruction, but a mismatched representation can select the wrong explanation.
- Learned imaging methods can be seen as replacing fixed sparse transforms with trainable representations.
- The central question becomes: who chose the representation, and why should it fit this imaging task?
